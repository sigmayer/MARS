...
public class Tools {
    ...
    public static ArrayList<int[]> makeKey(int[] k) {
        ...
        T.add(word);
        T.add(new int[]{0, 0, 0, k.length/4-1});
        for (int i = T.size(); i < 15; i++) {
            T.add(new int[4]);
        }
        ...
        for (int j = 0; j < 4; j++) {
            ...
            for (int l = 0; l < 4; l++) {
                for (int i = 0; i < T.size(); i++) {
                    int i1 = i - 1;
                    if (i1 < 0) { i1 += 15;}
                    int[] tmpT = modSum(T.get(i), Sbox(getBits(T.get(i1), 9)));
                    tmpT = CSP(tmpT, 9);
                    T.set(i, tmpT);
                }
            }
        ...
        for (int i = 5; i < 36; i+=2) {
            ...
            char[] binaryRes = new char[binaryNum.length];
            int count = 0;
            for (int c = 1; c < binaryNum.length; c++) {
                binaryRes[c] = '0';
                if (binaryNum[c] == binaryNum[c-1]) {
                    count++;
                }
                else {
                    if (count > 9) {
                        for (int o = c - count; o < c-1; o++) {
                            binaryRes[o] = '1';
                        }
                        count = 0;
                    }
            ...
            binaryRes[0] = '0';
            binaryRes[binaryRes.length-1] = '0';
            int M = (int)Long.parseLong(String.valueOf(binaryRes), 2);
            ...
            int[] p = CSL(B.get(j), getBits(key.get(i-1), 5));
            num = 0;
            for (int c = 0; c < 3; c++) {
                num ^= p[c];
                num = num << 8;
    ...
    public static ArrayList<int[]> E(int[] d, int[] k) {
        ...
        res3 = xor(res3, inverse(k));
        res3 = CSL(res3, 5);

        int bits = getBits(res2, 9);
        res1 = Sbox(bits);
        res1 = modSum(res1, res3);
    ...
    public static int[] Sbox(int d) {
        return S0(d);
    }

    public static int[] modSum(int[] slg1, int[] slg2) {
        if (slg1.length != slg2.length) {
        ...
        num1 += (long)slog1[0] << 24;
        num1 += (long)slog1[1] << 16;
        num1 += (long)slog1[2] << 8;
        num1 += (long)slog1[3];

        num2 += (long)slog2[0] << 24;
        num2 += (long)slog2[1] << 16;
        num2 += (long)slog2[2] << 8;
        num2 += (long)slog2[3];

        num1 += num2;
        num1 = num1 % (long)Math.pow(2, 32);

        int res[] = new int[4];
        for (int i = 3; i >= 0; i--) {
            res[i] = (int)num1 & 0b11111111;
            num1 = num1 >>> 8;
        }
        return res;
    }

    public static int[] modSub(int[] slg1, int[] slg2) {
        if (slg1.length != slg2.length) {
            System.out.println("Bruh");
            return null;
        }
    ...
    public static int[] xor(int[] d1, int[] d2) {
        for (int i = 0; i < 3; i++) {
            d1[i] ^= d2[i];
        }
        return  d1;
    ...
    public static int[] CSL(int[] D, int replace) {
        return CSP(D, 32-replace);
    }

    public static int[] CSP(int[] D, int replace) {
        ...
        for (int i = replace; i >= 0; i--) {
            newReplace += Math.pow(2, i);
        }

        tmpD = num & newReplace;
        num = num >>> replace;
        tmpD = tmpD << (32 - replace);
        num ^= tmpD;
    ...
    public static int getBits(int[] d, int cnt) {
        int num = 0;
        for (int i = 0; i < 3; i++) {
            num ^= d[i];
            num = num << 8;
